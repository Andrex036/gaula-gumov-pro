...
export default function CompletarRegreso() {
  ...
  const { data: todosRegistros = [], isLoading } = useQuery<any[]>({ queryKey: ["/api/registros"] });
  const pendientes = todosRegistros.filter((r: unknown) => (r as any).estado_registro === "Abierto");
  const registroSel = pendientes.find((r: any) => r.id === registroId) || pendientes[0];

  const mutation = useMutation({
    mutationFn: async ({ id, payload }: { id: string, payload: any }) => {
      const res = await fetch(`/api/registros/${id}/regreso`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      });
      if (!res.ok) throw new Error("Fallo al registrar regreso");
      return res.json();
    },
    onSuccess: () => {
      ...
      toast({
        ...
        description: "El vehículo se encuentra disponible en la base.",
        className: "bg-emerald-600 text-white border-none",
      });
      setLocation("/dashboard");
    },
    onError: () => {
      toast({ title: "Error", description: "Ocurrió un error al cerrar registro.", variant: "destructive" });
    }
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const km_regreso = parseFloat((formEl.elements.namedItem("km_regreso") as HTMLInputElement).value) || 0;
    const novedades = (formEl.elements.namedItem("novedades") as HTMLTextAreaElement).value;

    if (km_regreso < registroSel.kilometraje_salida) {
      toast({ title: "Error", description: "El KM de regreso no puede ser menor al de salida", variant: "destructive" });
      return;
    }

    const payload = {
      hora_regreso: format(new Date(), "HH:mm"),
      kilometraje_regreso: km_regreso,
      observaciones: novedades,
      fotos_regreso: fotos
    };

    mutation.mutate({ id: registroSel.id, payload });
  };

  if (isLoading) return <div className="text-center p-8">Cargando registros pendientes...</div>;
  ...
  return (
    <div className="pb-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Button type="submit" disabled={mutation.isPending} className="w-full h-14 text-lg bg-emerald-600 hover:bg-emerald-700 shadow-md" data-testid="button-guardar-regreso">
          <CheckCircle2 className="mr-2 h-5 w-5" /> {mutation.isPending ? "Cerrando..." : "Cerrar Registro"}
        </Button>
      </form>
    </div>
  );
